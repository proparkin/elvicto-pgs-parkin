...
class FlaskDetectNumberPlateSlotsCoordinateTwo implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try 
        {
            $response = Http::timeout($timeoutSeconds)->post('http://10.0.1.123:6000/process', [
                'current_folder' => '/root/slot_images/new_parkin_slot_images_501_1000',
            ...
            // ✅ Flask already returns JSON array (not nested under "data")
            $data = $response->json('data');
            
            if (empty($data) || !is_array($data)) 
            {
                Log::error("Invalid JSON received from Flask Two API");
                throw new \Exception("Invalid JSON received from Flask API");
            }
            
            Log::info("Second Flask API processed successfully. Total slots: " . count($data));

            // ✅ Continue your slot update logic
            $casesStatus = "";
            $casesVehicle = "";
            $casesImage = "";
            $casesChangeSlot = "";
        ...
        {
            Log::error("Flask Two DetectNumberPlateSlotsCoordinate Job Failed: " . $e->getMessage());
        }
        finally 
        {
